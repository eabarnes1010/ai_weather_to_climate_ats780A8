...
n_steps = 3  # number of timesteps in forecast
step = 6  # timestep in hours used for pangu forecast
output_data_dir = "output_data/"
output_zarr_dir = "output_data/"
# ------------------------------------------
...
for tt in range(0, n_steps):
    ...
    vtime = pd.date_range(
        start=vtime, end=vtime, freq="h"
    )  # this needs to be in an array rather than a single value

    pangu_sfc_arr = np.load(
        output_data_dir
        + "/"
    ...
    ds.attrs["source"] = (
        "Russ Schumacher, Colorado State University. Original Pangu-Weather code described in Bi et al. (2023, Nature) and obtained from https://github.com/198808xc/Pangu-Weather"
    )
    ...
    if tt == 0:  # start the zarr at first time, otherwise append
        ds.to_zarr(
            output_zarr_dir + "/" + init.strftime("%Y%m%d%H") + "/pangu_sfc.zarr",
            mode="w",
        )
    else:
        ds.to_zarr(
...
for tt in range(0, n_steps):
    ...
    if tt == 0:  # start the zarr at first time, otherwise append
        ds.to_zarr(
            output_zarr_dir + "/" + init.strftime("%Y%m%d%H") + "/pangu_upper.zarr",
            mode="w",
        )
    else:
        ds.to_zarr(
